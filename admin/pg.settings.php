...
<?php if($page_sub=='users'){
	...
	/* USERS END */
		...
		echo('<h1>'. $pg_title .'<span class="pull-right help-block"><small>'. setMyDate('now',3) .'</small></span></h1><hr>'.
			  $errText
			 );
...
}else if($page_sub=='update'){
		echo('<h1>'. letheglobal_update .'<span class="pull-right help-block"><small>'. setMyDate('now',3) .'</small></span></h1><hr>'.
			  $errText
			 );
			 
	/* UPDATE START */
	include_once('manage/lethe.update.php');
	/* UPDATE END */
		
}else if($page_sub=='language'){
		echo('<h1>'. letheglobal_language_editor .'<span class="pull-right help-block"><small>'. setMyDate('now',3) .'</small></span></h1><hr>'.
			  $errText
			 );
			 
	/* LANGUAGE START */
	include_once('manage/lethe.language.php');
	/* LANGUAGE END */
	...
	$pg_nav_buts = '<div class="nav-buts">
					...
					';
		echo('<h1>'. $pg_title .'<span class="help-block"><span class="text-primary">'. letheglobal_submission_accounts .'<span class="pull-right"><small>'. setMyDate('now',3) .'</small></span></span></span></h1><hr>'.
			  $pg_nav_buts.
			  $errText
			 );
