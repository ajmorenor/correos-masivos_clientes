...
/* 		$current = file_get_contents(LETHE."/newfile.txt");
		...
		$model='modelCsv';
		if(lethe_debug_mode){
			echo('<pre>DEBUG MODE: <br><br>');var_dump($fList);echo('</pre>');
		}
	}
	/* Create Array */
	if($model!='modelCsv'){
	...
	/* Mail Checker */
	$mailChk = $myconn->prepare("SELECT ID FROM ". db_table_pref ."subscribers WHERE OID=". set_org_id ." AND subscriber_mail=? AND GID=". $impGrp ."") or die(mysqli_error($myconn));
...
/* 								$jsonObject = $sub_mail;
								...
								$imp->bind_param('ssssssiisss',$sub_name,$sub_mail,$sub_web,$sub_date,$sub_tel,$sub_comp,$isActive,$isVerfiy,$subKey,$fullData,$subVerifyKey);
								$imp->execute();
								$recSc++;
								$sourceCntTemp++;
						}else{
		...
		/* Model CSV */
			...
			foreach($getCndToArr as $csvK=>$csvV){
				if($csvV=='subscriber_mail'){
					...
					$sub_date = ((array_key_exists($csvK,$v)) ? $v[$csvK]:'0000-00-00 00:00:00');
				}
				...
				else if($csvV=='subscriber_tag'){
					$sub_tag = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='local_country'){
					$sub_country = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='local_country_code'){
					$sub_country_code = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='local_city'){
					$sub_city = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='local_region'){
					$sub_region = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='local_region_code'){
					$sub_region_code = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
				else if($csvV=='ip_addr'){
					$sub_ip = ((array_key_exists($csvK,$v)) ? $v[$csvK]:null);
				}
			}
			...
			if(mailVal($sub_mail)){
						...
						# Blacklist control removed in v.2.1
					
						/* Check Data */
						$db->where('OID=? AND subscriber_mail=? AND GID=?',array(set_org_id,$sub_mail,$impGrp))->getOne('subscribers');
...
/* 								$jsonObject = $sub_mail;
								...
								$letheAdder = false;
								$letheAdder = $db->insert('subscribers',array(
																	'OID'=>set_org_id,
																	'GID'=>$impGrp,
																	'subscriber_name'=>$sub_name,
																	'subscriber_mail'=>$sub_mail,
																	'subscriber_web'=>$sub_web,
																	'subscriber_date'=>$sub_date,
																	'subscriber_phone'=>$sub_tel,
																	'subscriber_company'=>$sub_comp,
																	'subscriber_active'=>$isActive,
																	'subscriber_verify'=>$isVerfiy,
																	'subscriber_key'=>$subKey,
																	'subscriber_full_data'=>$fullData,
																	'subscriber_verify_key'=>$subVerifyKey,
																	'local_country'=>$sub_country,
																	'local_country_code'=>$sub_country_code,
																	'local_city'=>$sub_city,
																	'local_region'=>$sub_region,
																	'local_region_code'=>$sub_region_code,
																	'subscriber_tag'=>$sub_tag,
																	'ip_addr'=>$sub_ip
																));
								if($letheAdder){
									$recSc++;
...
if($pos=='export'){
	...
	if(!isset($_POST['csvOn']) || !is_numeric($_POST['csvOn'])){$csvOn=0;}else{{$csvOn=1;}}
	if(!isset($_POST['csvselection']) || !is_array($_POST['csvselection'])){$csvSels=array('subscriber_mail');}else{{$csvSels=$_POST['csvselection'];}}
	
	//die(var_dump($_POST['csvselection']));
	...
	if($resultData==''){
		...
		if($model=='csvOn'){
			# Create CSV Head
				$dts = array();
				foreach($csvSels as $csvK=>$csvV){
					$dts[] = $LETHE_CSV_EXP_FIELDS[$csvV];
				}
				$colData[] = implode($seperator,$dts);
		}
						...
						<div class="row">
							...
							<div class="col-md-6">
								...
								<div class="col-md-6">
									...
									<div class="alert alert-info">
										'. letheglobal_country .'
										<ul id="local_country" class="list-unstyled tablelist connectedSortable">

										</ul>
									</div>
									<div class="alert alert-info">
										'. letheglobal_country_code .'
										<ul id="local_country_code" class="list-unstyled tablelist connectedSortable">

										</ul>
									</div>
									<div class="alert alert-info">
										'. letheglobal_city .'
										<ul id="local_city" class="list-unstyled tablelist connectedSortable">

										</ul>
									</div>
									<div class="alert alert-info">
										'. letheglobal_region .'
										<ul id="local_region" class="list-unstyled tablelist connectedSortable">

										</ul>
									</div>
									<div class="alert alert-info">
										'. letheglobal_region_code .'
										<ul id="local_region_code" class="list-unstyled tablelist connectedSortable">
								...
								<div class="col-md-6">	
									...
									<div class="alert alert-info">
										'. letheglobal_subscriber_tag .'
										<ul id="subscriber_tag" class="list-unstyled tablelist connectedSortable">

										</ul>
									</div>
									<div class="alert alert-info">
										IP
										<ul id="ip_addr" class="list-unstyled tablelist connectedSortable">
