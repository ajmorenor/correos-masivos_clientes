...
if($pos=='DBTEST'){
	...
	if($getErr!=0){die('NO');}else{
		...
		@$myconns->query("SET @@SESSION.sql_mode='NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';");
		die('OK');
...
if($install){
	...
	if($getErr!=0){die('<div class="alert alert-danger">Database Connection Error!</div>');}else{
		...
		@$myconn->query("SET @@SESSION.sql_mode='NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION';");
...
?>";
		...
		die('<script>$("#myLethe").attr("disabled",true);</script>'.errMod('Lethe Successfully Installed on Your System!<br>Dont Forget to Remove <strong>install.php</strong> File!<br>Enter your <a href="'.lethe_root_url.'admin/" class="alert-link">Lethe Admin Panel</a>','success'));
	}else{
				...
				<div class="row">
					<div class="col-md-4">
						<div class="alert alert-danger">
							...
							Please check your Lethe root, you will see a <strong>.sql</strong> file.
						</div>
...
-->
					...
					<div class="col-md-4">
						...
						<div class="form-group">
							...
							<div class="input-group">
								...
								<span class="input-group-btn">
									<button class="btn btn-default" onclick="passShows('lethe_license_key');" type="button"><span class="glyphicon glyphicon-eye-close"></button>
								</span>
							</div>
						</div>
					</div>
	...
	<div class="lethe-footer">
		<p class="text-muted">
			<small>
			<a href="http://www.newslether.com" target="_blank">Lethe Newsletter &amp; Mailing System</a> v.<?php echo(LETHE_VERSION);?> &copy; 2015-<?php echo(date('Y'));?>.<br>
			Powered by <a href="http://www.artlantis.net/" target="_blank">Artlatis Design Studio</a>
			</small>
		</p>
	</div>
</div>
